...
SET AUTOCOMMIT=0;
USE `LoneClownTheory_blp`;
INSERT INTO `LoneClownTheory_blp`.`entityTable` (`entityID`, `entityName`, `subject_or_object`, `max_sensitivity`, `max_category`, `curr_sensitivity`, `curr_category`) VALUES (1, 'subject0', 1, 3,'CA,AZ,NM,TX,LAX,SFO,PHX,TUS,ABQ,IAH,DAL', 3,'CA,AZ,NM,TX,LAX,SFO,PHX,TUS,ABQ,IAH,DAL');
COMMIT;
...
CREATE  TABLE IF NOT EXISTS `LoneClownTheory_biba`.`entityTable` (
  ...
  `curr_sensitivity` INT UNSIGNED NOT NULL,
  `curr_category` SET('CA', 'AZ', 'NM', 'TX', 'SFO', 'LAX', 'PHX', 'TUS', 'ABQ', 'IAH', 'DAL'),
  `integrity` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`entityID`) ,
  UNIQUE INDEX `entityName_UNIQUE` (`entityName` ASC) )
ENGINE = InnoDB;
